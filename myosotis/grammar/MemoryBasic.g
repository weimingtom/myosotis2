...
('==' e2=expr { $value = Node.MakeNode(driver, Node.OP_EQ, $value, $e2.value); }
| '!=' e3=expr { $value = Node.MakeNode(driver, Node.OP_NE, $value, $e3.value); } 
| '>' e4=expr { $value = Node.MakeNode(driver, Node.OP_GT, $value, $e4.value); }
| '>=' e5=expr { $value = Node.MakeNode(driver, Node.OP_GE, $value, $e5.value); }
| '<' e6=expr { $value = Node.MakeNode(driver, Node.OP_LT, $value, $e6.value); }
| '<=' e7=expr { $value = Node.MakeNode(driver, Node.OP_LE, $value, $e7.value); }
)
;

expr returns [Node value]
: e1=mulexpr {$value = $e1.value;}
( '+' e2=mulexpr { $value = Node.MakeNode(driver, Node.OP_PLUS, $value, $e2.value); }
| '-' e3=mulexpr { $value = Node.MakeNode(driver, Node.OP_MINUS, $value, $e3.value); }
)*
;

mulexpr returns [Node value]
: e1=unaexpr {$value = $e1.value;}
( '*' e2=unaexpr { $value = Node.MakeNode(driver, Node.OP_TIMES, $value, $e2.value); }
| '/' e3=unaexpr { $value = Node.MakeNode(driver, Node.OP_DIVIDE, $value, $e3.value); }
| '%' e4=unaexpr { $value = Node.MakeNode(driver, Node.OP_MOD, $value, $e4.value); }
)*
;
...
| '-' e1=unaexpr { $value = Node.MakeNode(driver, Node.OP_NEG, $value); }
